...
from londiste.rest.sqltools import CopyToFilePipe
from londiste.util import find_copy_source
from skytools.dbstruct import *
from londiste.rest.playback import *

# import constants (e.g. TABLE_OK)
from londiste.playback import *

__all__ = ['CopyTableRest']

class CopyTableRest(ReplicatorRest):
    ...
    def __init__(self, args, copy_thread = 1):
        ...
        ReplicatorRest.__init__(self, args)
    ...
    def do_copy(self, tbl_stat, src_db, dst_db):
        ...
        file_buf = p.real_copy(src_real_table, src_curs, dst_curs, common_cols, self.work_dir)

        self.log.info("%s: csv copy finished to %s: %d bytes, %d rows, sending data to REST API",
                      tbl_stat.name, file_buf.filename, file_buf.total_bytes, file_buf.total_rows)

        try:
            rest = Rest(self.cf)
            rest.call_api('resync', os.path.realpath(file_buf.filename),
                          {'table': tbl_stat.name})
        finally:
            os.remove(file_buf.filename)
    ...
    def work(self):
        if not self.reg_ok:
            ...
            self.reg_ok = True
        return ReplicatorRest.work(self)
...
if __name__ == '__main__':
    script = CopyTableRest(sys.argv[1:])
    script.start()
